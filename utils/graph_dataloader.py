...
class GraphDataLoader(object):
    ...
    def __init__(self, statement_path, adj_path,
                 batch_size, device, model_name, max_node_num=200, max_seq_length=128,
        ...
        # Load data
        self.qids, self.labels, self.encoder_data, concepts_by_sents_list = self.load_input_tensors(statement_path,
                                                                                                    max_seq_length)

        num_choice = self.encoder_data[0].size(1)
        self.num_choice = num_choice
        assert num_choice > 0
        *self.train_decoder_data, self.adj_data = self.load_sparse_adj_data_with_contextnode(adj_path, max_node_num,
                                                                                             concepts_by_sents_list)
        assert all(len(self.qids) == len(self.adj_data[0]) == x.size(0) for x in
                   [self.labels] + self.encoder_data + self.train_decoder_data)
    ...
    def dev(self):
        ...
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.batch_size, indexes,
                                                   self.qids, self.labels, tensors0=self.encoder_data,
                                                   tensors1=self.train_decoder_data, adj_data=self.adj_data)
    ...
    def load_sparse_adj_data_with_contextnode(self, adj_pk_path, max_node_num, concepts_by_sents_list):
        ...
        if use_cache:
            with open(cache_path, 'rb') as f:
                adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type, half_n_rel, special_nodes_mask = utils.CPU_Unpickler(
                    f).load()
        else:
