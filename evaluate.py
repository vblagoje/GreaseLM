...
from utils import graph_dataloader
...
def load_data(args, devices, kg):
    ...
    dataset = graph_dataloader.GraphDataLoader(args.dev_statements, args.dev_adj,
...
def evaluate(args, devices, kg):
    ...
    print("Before args")
    print(args)
    d_args = utils.import_config(checkpoint["config"], args)
    d_args.dev_statements = dev_statements
    d_args.dev_adj = dev_adj

    print("After args")
    print(args)

    dataset = load_data(d_args, devices, kg)
    ...
    dev_total_loss, dev_acc = calc_eval_accuracy(dev_dataloader, model, "cross_entropy", loss_func)
